...
func (g *Goblin) isEnemy(entity game.Entity) bool {
	if _, isActive := entity.(game.EntityActive);
	!isActive {
		return false
	}
	if entityHealth, hasHealth := entity.(game.HasHealth);
	!hasHealth || entityHealth.GetHealth().CurrentHealth <= 0 {
		return false
	}
	if _, isGoblin := entity.(*Goblin);
	isGoblin {
		return false
	}
	return true
